...
auto
get_path_length(const Graph &g, const Container &con)
{
...
auto
get_cost(const Graph &g, const CU_Path<Graph> &p)
{
  return p.first.count() * get_path_length(g, p.second);
}

// Calculate the cost of a pair.
template<typename Graph, typename Path>
auto
get_cost(const Graph &g, const std::pair<Path, Path> &p)
{
  return get_cost(g, p.first) + get_cost(g, p.second);
}
...
calculate_utilization(const G &g)
{
  ...
  for (tie(ei, ee) = edges(g); ei != ee; ++ei)
    if (boost::get(boost::edge_weight, g, *ei) >= 0)
      {
        // Total available units.
        int tu = boost::get(boost::edge_nou, g, *ei);
        // Currenlty available units.
        int au = boost::get(boost::edge_su, g, *ei).count();
        // The link load.
        double load = double(tu - au) / tu;
        load_acc(load);
      }
